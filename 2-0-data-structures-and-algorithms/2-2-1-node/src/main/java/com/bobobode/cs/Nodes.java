...
public class Nodes {
     ...
     */
    public static <T> Node<T> link(Node<T> first, Node<T> second) {
    ...
    public static <T> Node<T> pairOf(T firstElement, T secondElement) {
        return new Node<T>(firstElement, new Node<T>(secondElement));
    }
    ...
    public static <T> Node<T> closedPairOf(T firstElement, T secondElement) {
        ...
        Node<T> secondNode = new Node<>(secondElement, firstNode);
    ...
    public static <T> Node<T> chainOf(T... elements) {
        ...
        Node<T> previosNode = firstNode;
        for (int i = 1; i < elements.length; i++) {
            previosNode=link(previosNode, new Node<>(elements[i]));
        }
        return firstNode;
    }
    ...
    public static <T> Node<T> circleOf(T... elements) {
        ...
        previosNode.setNextNode(firstNode);
        return firstNode;
    }
}
