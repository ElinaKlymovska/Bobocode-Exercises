...
public class CrazyGenerics {
     ...
     * @param <T> – value type
     */
    public static class MaxHolder<T extends Comparable<T>> implements Comparator<T> { // todo: refactor class to make it generic
        private T max;
     ...
     */
    interface ComparableCollection<E extends Comparable<? extends Collection<?>> & Collection<?>>
            extends Comparable<Collection<?>>, Collection<E> {
        @Override
        default int compareTo(Collection<?> collection) {
            return Integer.compare(this.size() - collection.size(), 0);
        }
    ...
    static class CollectionUtil {
        ...
        public static void swap(List<?> elements, int i, int j) {
            ...
            changePlaces(elements, i, j);
        }

        private static <T> void changePlaces(List<T> elements, int i, int j) {
            T elemant = elements.get(i);
            elements.set(i, elements.get(j));
            elements.set(j, elemant);
        }
    }
}
