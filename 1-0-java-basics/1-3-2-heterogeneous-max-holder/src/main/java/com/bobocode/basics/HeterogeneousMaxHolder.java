...
public class HeterogeneousMaxHolder {
    ...
    public <T extends Comparable<? super T>> T put(Class<T> key, T value){
        ...
        T currentValue = (T) store.get(key);
        if(currentValue == null){
            store.put(key,value);
            return null;
        }
        int resultComperemant = currentValue.compareTo(value);
        if(resultComperemant>=0){
            return value;
        } else {
    ...
    public <T> T put(Class<T> key, T value, Comparator<T> comparator){
        ...
        int resultComperemant = comparator.compare(currentValue,value);
        if(resultComperemant>=0){
            return value;
        } else {
            store.replace(key,value);
